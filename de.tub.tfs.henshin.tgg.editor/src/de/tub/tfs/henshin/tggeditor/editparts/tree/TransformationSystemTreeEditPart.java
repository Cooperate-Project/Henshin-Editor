...
public class TransformationSystemTreeEditPart extends AdapterTreeEditPart<TGG> {
	...
	protected void notifyChanged(Notification notification) {
		...
		default:
			// check that the TGGLayout of the transformation system is present
			// (e.g. can disappear when another editor is in use)
		...
		while (unitIter.hasNext()) {
			...
			if (!(unit instanceof IndependentUnit)) {
				...
				if (unit instanceof Rule) {
					if (TggUtil.getElemMarker(unit) == null) {
					...
					} else if (RuleUtil.TGG_RULE.equals(TggUtil.getElemMarker(unit))) {
						ruleFolder = module.getUnit("RuleFolder");

					} else if (RuleUtil.TGG_FT_RULE.equals(TggUtil.getElemMarker(unit))) {
						ruleFolder = module.getUnit("FTRuleFolder");

					} else if (RuleUtil.TGG_BT_RULE.equals(TggUtil.getElemMarker(unit))) {
						ruleFolder = module.getUnit("BTRuleFolder");
					} else if (RuleUtil.TGG_CC_RULE.equals(TggUtil.getElemMarker(unit))) {
						ruleFolder = module.getUnit("CCRuleFolder");
						// NEW
					} else if (RuleUtil.TGG_IT_RULE.equals(TggUtil.getElemMarker(unit))) {
						ruleFolder = module.getUnit("ITRuleFolder");
					} // NEW end
