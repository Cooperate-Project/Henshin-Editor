...
public class TggAggInfo extends AggInfo {
	...
	private void extendEdgeTypeMultiplicity(TGG layoutSystem) {
		...
		while (arcs.hasNext()) {
			...
			// TODO: determine triple component from tgg rule, because a type
			// may occur in several components
			if (NodeUtil.isCorrespondenceClass(layoutSystem, srcC) && NodeUtil.isSourceClass(layoutSystem, tarC)) {
			...
			if (r.getName().startsWith("FT") || r.getName().startsWith("CR")) {
				...
				for (TNode n : rhsNodes) {
					if (NodeUtil.isSourceNode(n)) {
						...
						if (RuleUtil.Translated.equals(TggUtil.getElemMarker(n)) && aggNode.getAttribute() != null
								&& aggNode.getAttribute().getMemberAt("translated") != null) {
							...
							for (Mapping m : r.getMappings()) {
								if (m.getImage() == n) {
									...
									agg.xt_basis.Node aggNodeLhs = (agg.xt_basis.Node) this.henshinToAggConversionMap
											.get(nLhs);
									if (aggNodeLhs.getAttribute().getMemberAt("translated") != null) {
										// TODO check if this is correct
										aggNodeLhs.getAttribute().setExprValueAt(
												String.valueOf(!RuleUtil.Translated.equals(TggUtil.getElemMarker(n))),
												"translated");
