...
/**
 * 
 * @author Jerry ConcurrentRuleList generates and stores all possible concurrent
 *         rules from a left rule and a right rule
 */
public class ConcurrentRuleList extends LinkedList<TGGRule> {
	...
	/**
	 * Constructor of List that contains all possible concurrent rule
	 * constructions from ruleLeft and ruleRight.
	 * 
	 * @param ruleLeft
	 * @param ruleRight
	 */
	public ConcurrentRuleList(final TGGRule ruleLeft, final TGGRule ruleRight) {
		...
		// ruleL is inverse of ruleLeft since we have to apply the inverse in
		// order to create LHS of concurrent rule
		...
		// generate all possible cospans (intersections) of the LHSides of both
		// ruleL and ruleR
		RuleLhsCoSpanList graphsLCoSpans = new RuleLhsCoSpanList(this.ruleL, this.ruleR);

		// clear Lhs attributes of rules because the matching target nodes are
		// already fixed,
		// and no problems with supposedly matching attributes will occur that
		// way
		...
		/// instead of expressions such as params in order to make rules
		...
		for (RuleLhsCoSpan graphsLCoSpan : graphsLCoSpans) {
			// copy the generated union C (center graph) of the corresponding
			// cospan twice
			// because we have to apply 2 rules to it, ruleL and ruleR
			Graph2GraphCopyMappingList graphCoSpanC2graphCoSpanCCopyL = new Graph2GraphCopyMappingList(
					graphsLCoSpan.getGraphCoSpanC());
			Graph2GraphCopyMappingList graphCoSpanC2graphCoSpanCCopyR = new Graph2GraphCopyMappingList(
					graphsLCoSpan.getGraphCoSpanC());
			...
			// after rule applications, graphCoSpanCCopyL/R will be lhs/rhs
			// graphs of resulting concurrent Rule respectively
			graphCoSpanCCopyL.setName("ConcurrentRuleLhs");
			graphCoSpanCCopyR.setName("ConcurrentRuleRhs");

			// replace attribute values with id values that represent real
			// target graph values
			/// instead of expressions such as params in order to make rules
			// applicable to the graphs
			...
			// construct mappings from CoSpanGraphs(L/R) to graphCCopy(L/R)
			// respectively
			MappingList graphLRuleL2graphCCopyL = new MappingComposition(graphsLCoSpan.getGraphCoSpanL2graphCoSpanC(),
					graphCoSpanC2graphCoSpanCCopyL, graphCoSpanCCopyL);
			MappingList graphLRuleR2graphCCopyL = new MappingComposition(graphsLCoSpan.getGraphCoSpanR2graphCoSpanC(),
					graphCoSpanC2graphCoSpanCCopyL, graphCoSpanCCopyL);

			MappingList graphLRuleL2graphCCopyR = new MappingComposition(graphsLCoSpan.getGraphCoSpanL2graphCoSpanC(),
					graphCoSpanC2graphCoSpanCCopyR, graphCoSpanCCopyR);
			MappingList graphLRuleR2graphCCopyR = new MappingComposition(graphsLCoSpan.getGraphCoSpanR2graphCoSpanC(),
					graphCoSpanC2graphCoSpanCCopyR, graphCoSpanCCopyR);

			for (Node nodeLRuleL : this.ruleL.getLhs().getNodes()) {
				Node nodeGraphCCopyL = graphLRuleL2graphCCopyL.getImage(nodeLRuleL, graphCoSpanCCopyL);
				EObject target = eGraphL.getNode2ObjectMap().get(nodeGraphCCopyL);
				matchL.setNodeTarget(nodeLRuleL, target);
			}

			for (Node nodeLRuleR : this.ruleR.getLhs().getNodes()) {
				Node nodeGraphCCopyR = graphLRuleR2graphCCopyR.getImage(nodeLRuleR, graphCoSpanCCopyR);
				EObject target = eGraphR.getNode2ObjectMap().get(nodeGraphCCopyR);
			...
			for (Node nodeCoSpanCCopyL : graphCoSpanCCopyL.getNodes()) {
				...
				if (nodeLRuleL != null) {
					...
					for (Attribute attributeNodeCoSpanCCopyL : nodeCoSpanCCopyL.getAttributes()) {
						if (RuleUtil.NEW.equals(TggUtil.getElemMarker(attributeNodeCoSpanCCopyL))) {
			...
			/*
			 * for (Node nodeCoSpanCCopyR : graphCoSpanCCopyR.getNodes()){ Node
			 * nodeLRuleL = graphLRuleL2graphCCopyR.getOrigin(nodeCoSpanCCopyR);
			 * Node nodeLRuleR =
			 * graphLRuleR2graphCCopyR.getOrigin(nodeCoSpanCCopyR); if
			 * (nodeLRuleL!=null && nodeLRuleR!=null){//if intersecting node
			 * HashSet<Attribute> atrributesNodeCoSpanCCopyRToRemove = new
			 * HashSet<Attribute>(); for (Attribute atrributeNodeCoSpanCCopyR :
			 * nodeCoSpanCCopyR.getAttributes()){ //if attribute is intersecting
			 * remove it if
			 * (AttributeUtil.getAttributeMarker(atrributeNodeCoSpanCCopyR)!=
			 * null &&
			 * AttributeUtil.getAttributeMarker(atrributeNodeCoSpanCCopyR).
			 * equals(RuleUtil.INTERSECTING)){
			 * atrributesNodeCoSpanCCopyRToRemove.add(atrributeNodeCoSpanCCopyR)
			 * ; } } nodeCoSpanCCopyR.getAttributes().removeAll(
			 * atrributesNodeCoSpanCCopyRToRemove); }else if (nodeLRuleL==null
			 * && nodeLRuleR!=null){//only part of right rule
			...
			for (Node nodeRRuleL : this.ruleL.getRhs().getNodes()) {
				for (Attribute attRnodeRRuleL : nodeRRuleL.getAttributes()) {
					TggUtil.setElemMarker(attRnodeRRuleL, RuleUtil.NEW);
			...
			for (Node nodeRRuleR : this.ruleR.getRhs().getNodes()) {
				for (Attribute attRnodeRRuleR : nodeRRuleR.getAttributes()) {
					TggUtil.setElemMarker(attRnodeRRuleR, RuleUtil.NEW);
					// ((TAttribute)attRnodeRRuleR).setMarkerType(RuleUtil.NEW);;
				}
			}

			RuleApplication ruleAppL = applyRule(this.ruleL, eGraphL, matchL);
			// check whether ruleL was successful, if not then the concurrent
			// rule is not valid
			if (ruleAppL != null && ruleAppL.getResultMatch() != null) {
				RuleApplication ruleAppR = applyRule(this.ruleR, eGraphR, matchR);
				if (ruleAppR == null || ruleAppR.getResultMatch() == null)
					continue;
				TGGRule concurrentRule = TggFactoryImpl.init().createTGGRule();
				// new TripleGraphImpl();
				concurrentRule
						.setName(ConcurrentRuleUtil.getConcurrentRuleName(ruleLeft, ruleRight, concurrentRuleIndex));
				// rule applications transformed the union into lhs and rhs of
				// concurrent rule respectively
				concurrentRule.setLhs(graphCoSpanCCopyL);
				concurrentRule.setRhs(graphCoSpanCCopyR);

				((TripleGraph) concurrentRule.getLhs())
						.setDividerCT_X(((TripleGraph) graphCoSpanCCopyL).getDividerCT_X());
				((TripleGraph) concurrentRule.getLhs())
						.setDividerSC_X(((TripleGraph) graphCoSpanCCopyL).getDividerSC_X());
				((TripleGraph) concurrentRule.getLhs())
						.setDividerMaxY(((TripleGraph) graphCoSpanCCopyL).getDividerMaxY());
				((TripleGraph) concurrentRule.getLhs())
						.setDividerYOffset(((TripleGraph) graphCoSpanCCopyL).getDividerYOffset());

				((TripleGraph) concurrentRule.getRhs())
						.setDividerCT_X(((TripleGraph) graphCoSpanCCopyR).getDividerCT_X());
				((TripleGraph) concurrentRule.getRhs())
						.setDividerSC_X(((TripleGraph) graphCoSpanCCopyR).getDividerSC_X());
				((TripleGraph) concurrentRule.getRhs())
						.setDividerMaxY(((TripleGraph) graphCoSpanCCopyR).getDividerMaxY());
				((TripleGraph) concurrentRule.getRhs())
						.setDividerYOffset(((TripleGraph) graphCoSpanCCopyR).getDividerYOffset());

				for (Node nodeRRuleL : this.ruleL.getRhs().getNodes()) {
					TNode concurrentNodeL = getAppResNodeFromRuleNode(ruleAppL, nodeRRuleL);
					// if (concurrentNodeL.getX()!=0 &&
					// concurrentNodeL.getX()!=((TNode)nodeRRuleL).getX()) throw
					// new IllegalArgumentException("remove this");
				...
				for (Node nodeRRuleR : this.ruleR.getRhs().getNodes()) {
					...
					// if (concurrentNodeR.getX()!=0) {System.out.println("x was
					// "+concurrentNodeR.getX()+ " and is
					// "+((TNode)nodeRRuleR).getX());};
					concurrentNodeR.setX(((TNode) nodeRRuleR).getX());
					concurrentNodeR.setY(((TNode) nodeRRuleR).getY());
				}

				MappingList graphCoSpanCCopyL2graphCoSpanC = ConcurrentRuleUtil
						.getInverseMappingList(graphCoSpanC2graphCoSpanCCopyL);
				MappingList unionL2unionR = new MappingComposition(graphCoSpanCCopyL2graphCoSpanC,
						graphCoSpanC2graphCoSpanCCopyR, concurrentRule.getRhs());
				...
				// TNode nodeLConcurrentRule =
				// getAppResNodeFromRuleNode(ruleAppL, origin);
				// TNode nodeRConcurrentRule =
				// getAppResNodeFromRuleNode(ruleAppR, image);
				// Test.out("ADD Mapping
				// 0:"+Test.outNodeRepresentation(nodeLConcurrentRule)+"-->"+Test.outNodeRepresentation(nodeRConcurrentRule));
				// concurrentRule.getMappings().add(nodeLConcurrentRule,
				// nodeRConcurrentRule);
				// }
				//
				...
				//
				// TNode nodeLConcurrentRule =
				// getAppResNodeFromRuleNode(ruleAppL, nodeRRuleL);
				// Test.check(nodeLConcurrentRule!=null);
				// if
				// (concurrentRule.getMappings().getImage(nodeLConcurrentRule,
				// concurrentRule.getRhs())==null){
				// Node nodeGraphCoSpanC =
				// ruleLhsCoSpan.getGraphRRuleL2GraphCoSpanC().getImage(nodeRRuleL,
				// ruleLhsCoSpan.getGraphCoSpanC());
				// Test.check(nodeGraphCoSpanC!=null);
				// //Node nodeRRuleR =
				// ruleLhsCoSpan.getGraphCoSpanC2graphRRuleR().getImage(nodeGraphCoSpanC,
				// ruleR.getRhs());
				// Node nodeRConcurrentRule =
				// graphCoSpanC2graphCoSpanCCopyR.getImage(nodeGraphCoSpanC);
				// Test.check(nodeRConcurrentRule!=null);
				// Test.out("ADD Mapping
				// 2:"+Test.outNodeRepresentation(nodeLConcurrentRule)+"-->"+Test.outNodeRepresentation(nodeRConcurrentRule));
				// concurrentRule.getMappings().add(nodeLConcurrentRule,
				// nodeRConcurrentRule);
				// }
				// }
				...
				// for (Node nodeLRuleR : this.ruleR.getLhs().getNodes()){
				// TNode nodeRConcurrentRule =
				// getAppResNodeFromRuleNode(ruleAppR,
				// ruleR.getMappings().getImage(nodeLRuleR, ruleR.getRhs()));
				// Test.check(nodeRConcurrentRule!=null);
				// if
				// (concurrentRule.getMappings().getOrigin(nodeRConcurrentRule)==null){
				// Node nodeRGraphC =
				// ruleLhsCoSpan.getGraphCoSpanR2graphCoSpanC().getImage(nodeLRuleR,
				// ruleLhsCoSpan.getGraphCoSpanC());
				// Test.check(nodeRGraphC!=null);
				// Node nodeLConcurrentRule =
				// graphCoSpanC2graphCoSpanCCopyL.getImage(nodeRGraphC);
				// if (nodeLConcurrentRule!=null){
				// Test.out("ADD Mapping
				// 3:"+Test.outNodeRepresentation(nodeLConcurrentRule)+"-->"+Test.outNodeRepresentation(nodeRConcurrentRule));
				// concurrentRule.getMappings().add(nodeLConcurrentRule,
				// nodeRConcurrentRule);
				// }
				// }
				// }
				/**
				 * for (Node concurrentRuleNodeL :
				 * concurrentRule.getLhs().getNodes()){ if
				 * (concurrentRule.getAllMappings().getImage(
				 * concurrentRuleNodeL, concurrentRule.getRhs())==null){
				 * 
				 * Node ruleRNodeR =
				 * ruleCospan.getGraphC2graphRRuleR().getImage(graphCCopyL.
				 * getOrigin(concurrentRuleNodeL),
				 * ruleCospan.getRuleR().getRhs()); TNode concurrentRuleNodeR =
				 * getAppNodeFromRuleNode(ruleRNodeR, ruleAppR); //TNode
				 * concurrentRuleNodeL = getAppNodeFromRuleNode(nLhs, ruleAppL);
				 * System.out.println("r l "+concurrentRuleNodeR);
				 * System.out.println("r l "+concurrentRuleNodeL);
				 * concurrentRule.getAllMappings().add(concurrentRuleNodeL,
				 * concurrentRuleNodeR); System.out.println("Good news"); }else{
				...
				Test.check(concurrentRule.getAttributeConditions()
						.isEmpty() == (attributeConditionsR.isEmpty() && attributeConditionsL.isEmpty()));
				...
				graphsLCoSpan.getIntersectionHandler().replaceAttributeHashValuesWithInitialExpressionsAndSetMarkers(
						concurrentRule, attributeID2Value);
		...
		if (ConcurrentRuleUtil.isConcurrent(ruleLeft, ruleRight) && this.isEmpty())
			;
		// throw new IllegalArgumentException("Generation of Concurrent Rules
		// Failed because of special case: "+ruleLeft.getName()+" merging with
		// "+ruleRight.getName());
	}
	...
	private static TNode getAppResNodeFromRuleNode(RuleApplication ruleApp, Node ruleNode) {
		...
		EObject ruleNodeTarget = match.getNodeTarget(ruleNode); // get resulting
																// nodeObject of
																// rule
																// applicatoin
	...
	private static RuleApplication applyRule(Rule rule, TggHenshinEGraph henshinGraph, Match match) {
		...
		// if (match==null || match.getRule()!=rule) throw new
		// IllegalArgumentException("match has to correspond to rule");
		// final TggHenshinEGraph henshinGraph = new TggHenshinEGraph(graph);
		/*
		 * Map<Node,EObject> node2eObject; Map<EObject, Node> eObject2Node;
		 * eObject2Node = henshinGraph.getObject2NodeMap(); node2eObject =
		 * henshinGraph.getNode2ObjectMap();
		 */
		...
		try {
			/*
			 * Iterator<Match> matchesIterator = emfEngine .findMatches(rule,
			 * henshinGraph, match//new MatchImpl(rule) ).iterator(); if
			 * (!matchesIterator.hasNext()) return null;
			 */
			...
			ex.printStackTrace();
			ErrorDialog.openError(Display.getDefault().getActiveShell(), "Execute Failure",
					"The rule [" + ruleApplication.getRule().getName() + "] couldn't be applied.",
					new Status(IStatus.ERROR, MuvitorActivator.PLUGIN_ID, ex.getMessage(), ex.getCause()));
			return null;
		}
	}
